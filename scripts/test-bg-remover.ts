...
runTest('Background Remover is properly registered in TOOL_REGISTRY', () => {
...
runTest('Background Remover route is registered in PSEO_ROUTES_LIST', () => {
  const seoRoute = PSEO_ROUTES_LIST.find(r => r.path === '/background-remover');
  assert.ok(seoRoute, 'Route should exist in SEO routes list');
});

// 3. Dimension safety clamping for normal images
runTest('Keeps normal resolution images untouched (e.g. 1920x1080)', () => {
...
// 4. Dimension safety clamping for extreme megapixel images (e.g. 48MP phone photo)
runTest('Clamps extreme megapixel images (8000x6000 = 48MP) to 24MP limit', () => {
...
// 5. Dimension safety clamping for extreme single-dimension banners (e.g. 12000x1000)
runTest('Clamps single extreme dimension (12000x1000) to max dimension ceiling', () => {
...
// 6. Filename generator for pipeline
runTest('Generates proper cutout filename based on format', () => {
...
// 7. ImageFileItem bridge conversion
runTest('Converts BgRemovalResult into standard ImageFileItem representation', () => {
